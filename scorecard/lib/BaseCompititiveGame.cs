...
using log4net;
public abstract class BaseCompititiveGame
{
    private CompititiveGameStatusPublisher statusPublisher;
    ...
    {
        LogData($"lifeline changed to:{newLifeline}");
    ...
    public BaseCompititiveGame(GameConfig co)
    {
        logger.Log("basegame constructor");
        this.config = co;
        statusPublisher = new CompititiveGameStatusPublisher(config.gameEngineIp); // Replace with your IP and port
